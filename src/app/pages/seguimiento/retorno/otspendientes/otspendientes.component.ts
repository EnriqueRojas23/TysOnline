...
@Component({
  selector: 'app-otspendientes',
  templateUrl: './otspendientes.component.html',
  styleUrls: ['./otspendientes.component.scss'],
...
export class OtspendientesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    [
        ...
        // {header: 'GRT', field: 'guiatransportista'  ,  width: '90px' },
        {header: 'OT', field: 'numcp'  ,  width: '90px' },
        {header: 'F. ENTREGA REAL', field: 'fechaentregareal' , width: '120px'  },
        {header: 'REMITENTE', field: 'remitente' , width: '120px'  },
        {header: 'DESTINO', field: 'destino'  ,  width: '90px'  },
        {header: 'ESTADO', field: 'estado' , width: '180px'  },

        {header: 'F. ENTREGA', field: 'fechaentrega' , width: '120px'  },
        {header: 'HR ENTREGA' , field: 'horaentrega'  , width: '120px'   },


        {header: 'LEAD TIME', field: 'leadTime' , width: '120px'  },
        {header: 'DIAS TRANSCURRIDOS', field: 'diasTranscurridos' , width: '120px'  },
        {header: 'REPARTIDOR ', field: 'repartidor' , width: '180px'  },
      ];
  ...
  this.model.criterio = "";
    ...
    this.estados.push({ value: 1,  label : 'Pendiente de retorno'});
    this.estados.push({ value: 2,  label : 'Con Retorno documentario'});
  ...
  editar(id) {
    ...
    const ref = this.dialogService.open(CamModalComponent, {
      header: '',
      width: '60%',
      height: '250px',
  ...
  onRowEditSave(order: OrdenTransporte) {
    ...
    if(order.estado === 'Pend Facturacion')
    {
      this.toastr.warning('Esta OT ya fue registrada con retorno documentario.'
    ...
    order.idtipoentrega = getIdTipoEntrega(order.tipoentrega)
    ...
    if (order.idordentrabajo > 0) {
        ...
        order.idusuarioentrega = this.model.idusuarioregistro;

        delete this.clonedOrders[order.idordentrabajo];

        this.ordenTransporteService.confirmar_liquidacionDoc(order).subscribe( resp => {


          this.toastr.success('Se confirmó correctamente la entrega documentaria'
          , 'Orden de Transporte', {
            closeButton: true
          });
...
onRowEditCancel(order: OrdenTransporte, index: number) {
  ...
  buscar() {
      ...
      this.ordenTransporteService.getAllOrderTransportDocuments(this.model).subscribe(list =>  {
        this.loading = false;
        this.ordenes =  list;
        this.ordenes2 = list;
...
function getIdTipoEntrega(tipoentrega: string) : number {
...
function getIdVisita(motivo1des: string): number {
  ...
  if (motivo1des === "Faltan datos de dirección")
  {
    return 29;
  }

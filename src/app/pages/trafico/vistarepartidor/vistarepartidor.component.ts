...
import { CambiarEstadoModalComponent } from '../vistaenruta/modalcambiarestado';
import { ModalReprogramarArribosComponent } from '../vistaenruta/modalreprogramararribos';
import { EventosModalComponent } from '../vistaenruta/modaleventos';
import { EntregarOtModalComponent } from './modalentregarOT';
import { ProgramarArribosModalComponent } from './modalProgramarArribos';
import { EnvioCargoModalComponent } from './modalenviocargoOT';


@Component({
  selector: 'app-vistarepartidor',
  templateUrl: './vistarepartidor.component.html',
  styleUrls: ['./vistarepartidor.component.scss'],
  ...
  this.router.navigate(['/seguimiento/verorden', idordentrabajo]);
   ...
   reloadDetalles() {
    ...
    this.generalService.getProveedor(this.id).subscribe( resp => {
      ...
      this.NameRepartidor =   resp.razonSocial;
      this.NameProvincia = resp.distrito;

    });


    this.ordenService.GetAllOrdersxRepartidorGroupProvincias (this.id).subscribe(list => {
      this.despachos = list;
    ...
    this.ordenService.getAllOrdersxRepartidor(this.id, 13).subscribe(x => {
      this.ordenes2 = x;
    ...
    this.ordenService.getAllOrdersxRepartidor(this.id, 34).subscribe(x => {
      this.ordenes3 = x;
      console.log(this.ordenes2);
    ...
    this.ordenService.getAllOrdersxRepartidor(this.id, 35).subscribe(x => {
      this.ordenes4 = x;
      console.log(this.ordenes2);
